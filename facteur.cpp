...
QSqlQueryModel* Facteur::rechercher(QString a)
 {


    QSqlQueryModel *model=new QSqlQueryModel();

    /* model->setQuery("select * from FACTURE where ref LIKE '%"+a+"%' OR datefact LIKE '%"+a+"%' OR type LIKE '%"+a+"%' "
         "OR  montant LIKE '%"+ a +"%' ");  //comme un tableau de données en ligne , on l affiche sur table view
      //ajouter a à la requette a l'aide de + // colonne =a */

    model->setQuery("select * from FACTURE where ref LIKE '"+a+"%' OR  type LIKE '"+a+"%' "
             );
     model->setHeaderData(0, Qt::Horizontal, QObject::tr("Référence"));
     model->setHeaderData(1, Qt::Horizontal,QObject::tr("Date"));
     model->setHeaderData(2, Qt::Horizontal,QObject::tr("Type"));
